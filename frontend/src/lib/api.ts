...
export const api = {
  ...
  createAlert: (body: { symbol: string; condition: string; threshold: number; email?: string; note?: string; recurring?: boolean; webhook_url?: string; compound_conditions?: CompoundCondition[] }) =>
...
export type HoldDaysSummary = { min: number; median: number; max: number };
export type SignalAccuracyReport = { lookback_days: number; total_signals: number; buy_count: number; sell_count: number; buy_accuracy: number | null; sell_accuracy: number | null; overall_accuracy: number | null; avg_buy_return_pct: number | null; avg_sell_return_pct: number | null; hold_days_buy: HoldDaysSummary | null; hold_days_sell: HoldDaysSummary | null; profit_factor: number | null; page: number; page_size: number; has_more: boolean; signals: SignalAccuracyRow[] };
...
export type OutcomesSummary = {
  ...
  by_direction?: Record<string, { count: number; win_rate: number; avg_return_pct: number | null; reliable: boolean }>;
