        ...
        <div>
          ...
          {!outcomesData || outcomesData.total === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              {/* AUD261-PAPERTRADE-PANEL-MISLABEL (fixed 2026-08-06): this panel previously
                  claimed "actual closed trades written back by PT-J1" — verified false by
                  grep: paper_trading_engine.py never writes return_5d/is_correct_5d/
                  return_10d/return_20d. Those columns are written solely by
                  evaluate_signal_outcomes()'s _window_return() — hypothetical forward returns
                  from the signal's own entry price, with no stops, no position sizing, and no
                  real exits. win_rate here IS direction-aware (is_correct_Nd already applies
                  the BUY/SELL hurdle correctly); avg_return_pct was NOT until the sibling
                  AUD261-OUTCOMESSUMMARY-UNSIGNED-SELL fix (now sign-corrected via
                  _signed_return() on the backend). Relabeled to describe what this actually
                  is, and "trades"/"signals" corrected below. */}
              {outcomesData.by_window && Object.values(outcomesData.by_window).some(v => v !== null) && (
                <div style={{ marginTop: 16 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: '#94a3b8', marginBottom: 8, letterSpacing: '0.05em', textTransform: 'uppercase' }}>
                    Forward Return by Hold Window <span style={{ fontWeight: 400, fontSize: 11, color: '#475569' }}>· hypothetical — no stops, no position sizing, no real exits</span>
                  </div>
                    ...
                    {(['5d', '10d', '20d'] as const).map(w => {
                      ...
                      return (
                        <div key={w} style={{ flex: '1 1 120px', minWidth: 110, background: '#0a0f1a', border: '1px solid #1e293b', borderRadius: 8, padding: '10px 14px' }}>
                          ...
                          <div style={{ fontSize: 10, color: '#475569', marginTop: 2 }}>win rate · {v.count} signal{v.count !== 1 ? 's' : ''}</div>
