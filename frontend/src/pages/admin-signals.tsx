...
export default function AdminSignalsPage() {
  ...
  // Refresh
  const [refreshing, setRefreshing] = useState<'US' | 'HK' | null>(null);
  const [refreshMsg, setRefreshMsg] = useState<string | null>(null);

  async function handleRefresh(market: 'US' | 'HK') {
    setRefreshing(market);
    setRefreshMsg(null);
    try {
      const r = await api.refreshSignals(market);
      setRefreshMsg(`${market} refresh triggered — ${r.count ?? '?'} signals queued`);
    } catch {
      setRefreshMsg(`${market} refresh failed`);
    }
    setRefreshing(null);
  }
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
            {(['US', 'HK'] as const).map(mkt => (
              <button
                key={mkt}
                onClick={() => handleRefresh(mkt)}
                disabled={refreshing !== null}
                style={{ padding: '7px 16px', borderRadius: 8, border: '1px solid rgba(129,140,248,0.35)', background: 'rgba(129,140,248,0.08)', color: refreshing === mkt ? '#f59e0b' : '#818cf8', cursor: refreshing !== null ? 'default' : 'pointer', fontSize: 12, fontWeight: 700, opacity: refreshing !== null && refreshing !== mkt ? 0.5 : 1 }}
              >
                {refreshing === mkt ? `Refreshing ${mkt}…` : `↻ Refresh ${mkt}`}
              </button>
            ))}
            {refreshMsg && <span style={{ fontSize: 12, color: '#94a3b8', alignSelf: 'center' }}>{refreshMsg}</span>}
          </div>
        </div>
        <button
