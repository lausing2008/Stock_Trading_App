...
- **`dsl/evaluator.py::compute_features()` reimplements RSI/MACD/ATR/Bollinger from scratch
  instead of calling the canonical `technical-analysis` service.** Confirmed 2026-07-03: the RSI
  formulas provably differ — `technical-analysis` has an explicit NaN-vs-zero disambiguation fix
  that this file's from-scratch copy lacks (`rs = g / l.replace(0, np.nan)` with no equivalent
  masking). Backtests here compute slightly different indicator values than the live signals
  generated by signal-engine for the same symbol/day — a real, unresolved discrepancy, not just
  a style preference. If you're debugging "backtest doesn't match what actually happened live,"
  check this divergence first.
