"""Watchlist endpoints — add/remove/list watched symbols."""
from fastapi import APIRouter, Depends, HTTPException
from pydantic import BaseModel
from sqlalchemy import select
from sqlalchemy.orm import Session

from db import Stock, WatchlistItem, get_session
from ..api.routes import StockOut
...
def add_to_watchlist(symbol: str, session: Session = Depends(get_session)):
    ...
    item = WatchlistItem(stock_id=stock.id)
    session.add(item)
    session.commit()
