"""Tests for T234-CONFIG-UNJUSTIFIED-THRESHOLDS item #23's new GET /backtest/open-risk-cap-sweep
route in paper_portfolio.py. Matches test_backtest_drawdown_sweep_route.py's own established
precedent exactly (its sibling /backtest/drawdown-breaker-sweep route) — this file only guards
the route's own wiring via source-text checks, since paper_portfolio.py can't be imported
directly in this test environment (conftest.py stubs sqlalchemy/db wholesale). The underlying
sweep_max_open_risk_pct() function itself is already thoroughly tested in
...
def _route_body():
    start = _PP_SOURCE.index('@router.get("/backtest/open-risk-cap-sweep")')
...
def test_route_is_registered_with_the_expected_path():
    assert '@router.get("/backtest/open-risk-cap-sweep")' in _PP_SOURCE
...
def test_route_delegates_to_sweep_max_open_risk_pct_not_a_reimplementation():
    body = _route_body()
    assert "from ..backtest.portfolio_backtest import sweep_max_open_risk_pct" in body
    assert "sweep_max_open_risk_pct(" in body
...
def test_route_uses_the_same_longer_default_window_as_its_sibling_sweeps():
    ...
    top of the outcome-resolution lag — matching the drawdown/risk-per-trade sweeps' own
