"""Quantitative scoring — technical, fundamental, DCF, checklist, position sizing.

T233-ARCH-INSERVICE-SPLITS: extracted verbatim from api/routes.py, which had grown to
1877 lines bundling report aggregation/orchestration with these independently-testable
pure functions — services/research-engine/tests/test_scoring.py already imported several
of them directly with zero FastAPI/network dependency, proving they were already decoupled
in practice, just not in file layout. No behavior change; routes.py re-imports these names
so every existing `from src.api.routes import X` call site (routes and tests alike) keeps
working unchanged.
"""
