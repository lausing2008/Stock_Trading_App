...
- **`docs/incidents/wire-shape-mismatches.md`** — `/events/overview`'s Nested `top_buys` Is a DIFFERENT Shape Than the Standalone Leaderboard Endpoints — Reused the Wrong Type; AUD-HORIZONCOMPARE-BEARREGIME — a reference page showing a hardcoded snapshot of values the backend OVERRIDES at runtime (symptomless: renders cleanly, numbers plausible, nothing errors). Read its 4-point checklist before adding or reviewing any "reference values" page.
