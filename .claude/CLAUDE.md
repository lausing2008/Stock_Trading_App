...
- **`docs/audits/2026-09-08-gate-audit.md`** — **ALL-GATES audit (2026-09-08): 12 findings, 1 fixed (tier 368), run as 3 parallel audits (entry / exit+risk / signal+alert) over 158 gate constants in 14 files.** **READ THE PATTERN SECTION FIRST — it is more valuable than any single finding.** THREE findings in one session were the SAME failure: **a fix that lands on the SHADOW path, not the deciding path.** `grep breakout_ref services/decision-engine/` → 0; `grep expected_move services/decision-engine/` → 0. It keeps happening because `paper_trading_engine._should_enter()` is the shadow-logged FALLBACK (`decision_engine_mode` defaults to `"primary"`) yet is the more readable, better-tested, more obvious place to fix — and a fix there produces plausible shadow logs while changing no real decision. **THE CHECK: after fixing any gate, grep the fixed symbol under `services/decision-engine/` and confirm it appears.** **FIXED — `AUD-SIGALERT-RRUNREACHABLE` (CRITICAL): BUY signal alerts were COMPLETELY DEAD for 4 days** (72h: 123 conviction passes, **0** DE-gate passes; WAIT/HOLD/SELL last sent 2026-09-08 vs BUY 2026-09-04) because a hardcoded 2.0 R:R target pinned `rr_ratio ≤ 2.00` while `calibrate_min_rr_ratio` raised the floor to 2.25 — two correct mechanisms, jointly lethal. **Scope corrected by my own test: GROWTH+LONG fully unblocked (64%), SWING only at low ATR, SHORT never** — I had claimed 83% from a single ATR spot-check; on fixed-stop geometry every style caps at ≤2.00 and ~62% of BUYs sit where that binds. **SHORT deferred by user decision** — its 3%/5% params cap R:R at 1.67 BY DESIGN, so blocking it is now CORRECT; forcing it would fabricate an unsupported target. **STILL OPEN, ranked:** `AUD-EXIT-INDICATORSEMPTY` (CRITICAL — 2 exit gates read the `indicators` table which has **0 rows and no writer anywhere**; the RSI data is in `signals.reasons` instead, 43,024 rows), `AUD-ENTRY-CONSECLOSS-DEADLOCKLOOP` (HIGH — the consecutive-loss breaker is permanently bypassed at zero open positions and re-grants every cycle; **portfolio 5 took 7 trades under it and all 7 lost, −$452.92**), `AUD-ENTRY-BREAKOUTREF-DEONLY` (HIGH), plus 8 MEDIUM/LOW. **CLEAN — do not re-derive:** stop machinery (monotonic across 7 tighteners, `current_stop < stop_loss` = 0 rows all-time, fills within 0.8%), the conviction gate (every layer fires; `roc_10` blocks 17.7%), all 2026-09-07 config divergences verified fixed and live, the watchdog now correctly TIGHTENING, and two things that look broken and are not (an `atr` that appears to leak across iterations; `weekly_confidence`'s 1.0 default, which the `weekly_rsi is None` short-circuit prevents from ever applying).
